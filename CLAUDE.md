...
- **Use-by vs best-before**: resolved. `products.date_type` / `batches.date_type` (`'use_by' | 'best_before'`) split the single expiry column — selling past a use-by date is a criminal offence in the UK, past best-before is routine. The till (`checkout()`) refuses to allocate stock from an expired use-by batch; best-before past date is advisory only, shown on the dashboard and product page as a markdown prompt rather than a block. Markdowns are now built (migration 0017, with the Figma redesign): a manager sets a reduced price on one batch, and the till charges it only when FEFO sells from that batch — one sale line per price, never a blended unit price. It stays a manager's decision per batch, not an automatic rule, and it never unlocks an expired use-by batch.
